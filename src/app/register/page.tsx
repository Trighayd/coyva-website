...
export default function RegisterPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (result?.ok) { router.push('/dashboard') } else { setError('Account created — please sign in'); setLoading(false) }
  }
      ...
      <div style={{ minHeight:'100vh', background:'#080809', display:'flex', alignItems:'center', justifyContent:'center', padding:20 }}>
        <div style={{ width:'100%', maxWidth:360, animation:'fadeUp 0.5s ease both' }}>
          ...
          <div style={{ textAlign:'center', marginBottom:32 }}>
            ...
            <h1 style={{ fontFamily:"'Syne',sans-serif", fontSize:22, fontWeight:700, color:'#fff', letterSpacing:'-0.8px', marginBottom:5 }}>Create your account</h1>
            <p style={{ fontSize:12, color:'rgba(255,255,255,0.25)', fontWeight:300 }}>Free to use · No credit card required</p>
          </div>

          {/* Form card */}
          <div style={{ background:'rgba(255,255,255,0.025)', border:'1px solid rgba(255,255,255,0.07)', borderRadius:16, padding:26 }}>
            <form onSubmit={handleSubmit}>
              <label style={{ display:'block', fontSize:9, color:'rgba(255,255,255,0.25)', letterSpacing:'1.5px', textTransform:'uppercase', marginBottom:7, fontFamily:"'Geist Mono',monospace" }}>Name (optional)</label>
              <input className="inp" type="text" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="Hayden" />

              <label style={{ display:'block', fontSize:9, color:'rgba(255,255,255,0.25)', letterSpacing:'1.5px', textTransform:'uppercase', marginBottom:7, fontFamily:"'Geist Mono',monospace" }}>Email</label>
              ...
              <label style={{ display:'block', fontSize:9, color:'rgba(255,255,255,0.25)', letterSpacing:'1.5px', textTransform:'uppercase', marginBottom:7, fontFamily:"'Geist Mono',monospace" }}>Password</label>
          ...
          <p style={{ textAlign:'center', fontSize:12, color:'rgba(255,255,255,0.2)', marginTop:18, fontWeight:300 }}>
            ...
            <a href="/login" style={{ color:'#c8ff00', fontWeight:500, textDecoration:'none', opacity:0.85 }}>Sign in</a>
          </p>
        </div>
      </div>
    </>
