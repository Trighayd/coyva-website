...
const CAT: Record<string, { label: string; color: string; budget: number }> = {
  groceries:     { label: 'Groceries',     color: '#22c55e', budget: 600 },
  transport:     { label: 'Transport',     color: '#3b82f6', budget: 200 },
  dining:        { label: 'Dining',        color: '#f97316', budget: 250 },
  utilities:     { label: 'Utilities',     color: '#eab308', budget: 180 },
  health:        { label: 'Health',        color: '#a855f7', budget: 100 },
  shopping:      { label: 'Shopping',      color: '#ec4899', budget: 200 },
  entertainment: { label: 'Entertainment', color: '#06b6d4', budget: 150 },
  income:        { label: 'Income',        color: '#34d399', budget: 0 },
  transfer:      { label: 'Transfer',      color: '#64748b', budget: 0 },
  other:         { label: 'Other',         color: '#64748b', budget: 100 },
}
...
export default function DashboardPage() {
  ...
  const fetchData = async () => {
    setLoading(true)
    try { const res = await fetch(`/api/transactions?month=${month}&year=${year}`); setData(await res.json()) } catch {}
    setLoading(false)
  }
  ...
  const filtered = transactions.filter(t => !search || cleanDesc(t.description).toLowerCase().includes(search.toLowerCase()))
    ...
    <>
      <style suppressHydrationWarning dangerouslySetInnerHTML={{ __html: `
        ...
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        .sp-row{display:flex;align-items:center;gap:11px;padding:9px 10px;border-radius:9px;cursor:pointer;transition:background 0.1s}
        ...
        .search-inp{background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.07);border-radius:7px;padding:5px 11px;color:rgba(255,255,255,0.6);font-size:12px;outline:none;font-family:'Geist',sans-serif;width:170px;transition:border-color 0.15s}
              ...
              <>
                        ...
                        {expenseCats.length === 0 ? (
                          ...
                          <div style={{ display:'flex', flexDirection:'column', gap:1 }}>
                            {expenseCats.map(([cat, amt], i) => {
                              ...
                              const over = meta.budget > 0 && (amt as number) > meta.budget
                              const pct = meta.budget > 0 ? Math.min((amt as number) / meta.budget * 100, 100) : 50
                              return (
                                  ...
                                  <div style={{ flex:1, minWidth:0 }}>
                                    <div style={{ display:'flex', justifyContent:'space-between', marginBottom:4 }}>
                                      ...
                                      <span style={{ fontFamily:"'Geist Mono',monospace", fontSize:11, color: over ? 'rgba(248,113,113,0.6)' : 'rgba(255,255,255,0.35)' }}>
                                        {fmt(amt as number)} {meta.budget > 0 && <span style={{ opacity:0.5 }}>/ {fmt(meta.budget)}</span>}
                                      </span>
                                    </div>
                ...
                {tab === 'transactions' && (
                  <div className="pn" style={{ padding:'18px', animation:'fadeUp 0.4s ease both', opacity:0 }}>
                    <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:16 }}>
                      <div style={{ fontFamily:"'Syne',sans-serif", fontSize:13, fontWeight:600, letterSpacing:'-0.3px', color:'rgba(255,255,255,0.7)' }}>{filtered.length} transactions</div>
