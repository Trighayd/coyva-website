...
// Manually trigger a transaction refresh from the bank.
// In production: also run this on a cron (e.g. daily at 6am AEST).
...
export async function POST() {
  ...
  // Check consent hasn't expired (CDR compliance)
  if (new Date() > connection.consentExpires) {
    await prisma.bankConnection.update({
      where: { id: connection.id },
      data:  { consentActive: false },
    })
    return NextResponse.json(
      { error: 'consent_expired', message: 'Please reconnect your bank to renew consent.' },
      { status: 403 }
    )
  }

  try {
    // Tell Basiq to pull latest data from the bank
    const jobId = await refreshUserData(connection.basiqUserId)

    // Give Basiq ~10s to complete the refresh (for demo purposes)
    // Production: poll /jobs/{jobId} until complete, or use webhooks
    ...
    for (const txn of transactions) {
      ...
      // Count truly new records (createdAt ≈ updatedAt on first write)
